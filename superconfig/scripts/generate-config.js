...
/**
 * Read superconfig options from the app's package.json
 */
function readSuperconfigOptions(appRootDir) {
    try {
        const packageJsonPath = path.join(appRootDir, 'package.json');
        if (fs.existsSync(packageJsonPath)) {
            const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'));
            if (packageJson.superconfig) {
                return {
                    injectBuildVars: !!packageJson.superconfig.injectBuildVars
                };
            }
        }
    } catch (error) {
        // Ignore errors reading options
    }
    return { injectBuildVars: false };
}
...
const options = readSuperconfigOptions(appRoot);
...
console.log('  injectBuildVars:', options.injectBuildVars);
...
/**
 * Escape a value for xcconfig format
 * xcconfig values with spaces or special characters should be quoted
 */
function escapeForXcconfig(str) {
    if (str === null || str === undefined) return '';
    // Replace newlines with spaces (xcconfig doesn't support multiline)
    let value = str.toString().replace(/\r?\n/g, ' ');
    return value;
}

/**
 * Generate xcconfig file content for Xcode build settings (Info.plist substitution)
 */
function generateXcconfig(config) {
    const lines = ['// Auto-generated by superconfig - DO NOT EDIT MANUALLY'];
    for (const [key, value] of Object.entries(config)) {
        lines.push(`${key} = ${escapeForXcconfig(value)}`);
    }
    return lines.join('\n') + '\n';
}
 ...
 */
function escapeForProperties(str) {
    if (str === null || str === undefined) return '';
    return str.toString()
        .replace(/\\/g, '\\\\')
        .replace(/\n/g, '\\n')
...
/**
 * Generate Java properties file content for Android manifestPlaceholders
 */
function generatePropertiesFile(config) {
    const lines = ['# Auto-generated by superconfig - DO NOT EDIT MANUALLY'];
    for (const [key, value] of Object.entries(config)) {
        lines.push(`${key}=${escapeForProperties(value)}`);
    }
    return lines.join('\n') + '\n';
}
...
if (fs.existsSync(envPath)) {
    ...
    // Generate build var files when opted in
    if (options.injectBuildVars) {
        console.log('[Superconfig] ⚠️  injectBuildVars is enabled — env values will be visible in built artifacts (IPA/APK)');

        // iOS: Generate xcconfig for Info.plist substitution
        // Write to library root so the podspec can read it via user_target_xcconfig
        const libraryXcconfigPath = path.join(libraryRoot, 'superconfig-env.xcconfig');
        const xcconfigContent = generateXcconfig(config);
        fs.writeFileSync(libraryXcconfigPath, xcconfigContent, 'utf-8');
        console.log('[Superconfig] ✅ Generated superconfig-env.xcconfig');

        // Android: Generate properties file for manifestPlaceholders (in app's android dir)
        const androidDir = path.join(appRoot, 'android');
        if (fs.existsSync(androidDir)) {
            const propertiesPath = path.join(androidDir, 'superconfig-env.properties');
            const propertiesContent = generatePropertiesFile(config);
            fs.writeFileSync(propertiesPath, propertiesContent, 'utf-8');
            console.log('[Superconfig] ✅ Generated android/superconfig-env.properties');
        }
    }
} else {
